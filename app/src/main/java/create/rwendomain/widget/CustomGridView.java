...
import create.rwendomain.util.CommUtil;
...
public class CustomGridView extends LinearLayout {
	...
	private void initView(Context context) {
		...
		viewWidth = (mContext.getResources().getDisplayMetrics().widthPixels - CommUtil.dip2px(mContext, 2)) / CustomGroup.COLUMNUM1;
	}
	...
	public void notifyDataSetChange(boolean needAnim) {
		removeAllViews();
		rowNum = mPlayList.size() / CustomGroup.COLUMNUM1 + (mPlayList.size() % CustomGroup.COLUMNUM1 > 0 ? 1 : 0);
		...
		for (int rowIndex = 0; rowIndex < rowNum; rowIndex++) {
			...
			for (int columnIndex = 0; columnIndex < CustomGroup.COLUMNUM1; columnIndex++) {
				int itemInfoIndex = rowIndex * CustomGroup.COLUMNUM1 + columnIndex;
				...
				View root = View.inflate(mContext, R.layout.gridview_child_layoutview, null);
				TextView textView = (TextView) root.findViewById(R.id.gridview_child_name_tv);
				final TextView tv_copy = (TextView) root.findViewById(R.id.tv_copy);
				if (isValidateView) {
					final DargChildInfo tempChilid = mPlayList.get(itemInfoIndex);
					if (tempChilid.getName().equals("在线转入")){
						tv_copy.setText("前往");
					}else if ((tempChilid.getName().contains("电话"))){
						tv_copy.setText("拨打");
					}else if ((tempChilid.getName().contains("QQ"))){
						tv_copy.setText("复制");
					}
					else {
						tv_copy.setText("去续费");
				...
				if (columnIndex != CustomGroup.COLUMNUM1 - 1) {
